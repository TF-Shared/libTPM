		   ...
		   uint32_t digest_len)
{
	...
	ret = tpm_update_buffer(&pcr_extend_cmd, TPM_MIN_AUTH_SIZE,
				sizeof(uint32_t));
	if (ret < 0) {
		return ret;
	}
	...
	/* hmac/password size */
	ret = tpm_update_buffer(&pcr_extend_cmd, TPM_ZERO_HMAC_SIZE,
				sizeof(uint16_t));
	if (ret < 0) {
		return ret;
	}
	...
	/* hashes count */
	ret = tpm_update_buffer(&pcr_extend_cmd, TPM_SINGLE_HASH_COUNT,
				sizeof(uint32_t));
	if (ret < 0) {
		return ret;
	}
	...
	for (int i = 0; i < digest_len; i++) {
		ret = tpm_update_buffer(&pcr_extend_cmd, digest[i],
					sizeof(uint8_t));
		if (ret < 0) {
			return ret;
		}
